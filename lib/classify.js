...
//
// V11.7: a TRANSFER out is also allowed on a debit — as long as it names where the money
// went (e.g. Sparkasse → your credit card, i.e. paying the card bill). It still leaves the
// bank account, so the bank balance is unchanged; the card simply receives it. V11.3
// clamped every transfer to "debit" and dropped the destination, so card bills paid from
// a synced account never reached the card.
...
function applyDecision(dec, dir) {
  ...
  const outTransfer = dir === "out" && dec.type === "transfer" && !!dec.toAccountId;
  const kept = (outTransfer || allowed.includes(dec.type)) ? dec.type : allowed[allowed.length - 1]; // credit / debit
  const clamped = kept !== dec.type;
  const { toAccountId, ...rest } = dec;
  return {
    ...rest, type: kept, ...(outTransfer ? { toAccountId } : {}),
    reviewed: true, needsReview: false, confidence: 1,
    reason: clamped
