...
export async function sendPasswordReset(email) {
  ...
  if (error) {
    ...
    throw authError("other", readableAuthError(error, "Couldn't send the reset email right now. Please try again in a few minutes."));
  }
}
// supabase-js reports an outright auth-server failure — a 500, such as the mail
// server refusing to send — with "{}" as its message. Anything that is not a real
// sentence gets one; genuine messages pass through untouched.
export function readableAuthError(error, fallback) {
  const m = (error?.message || "").trim();
  if (error?.status === 429 || /rate limit|too many/i.test(m)) return "Too many requests — wait a minute and try again.";
  if (!m || m === "{}" || (error?.status || 0) >= 500) return fallback;
  return m;
}
