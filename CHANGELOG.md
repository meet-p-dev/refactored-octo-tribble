...
- **New accounts start today automatically**, including the ones you add during setup. Adding older transactions later fills in your history without breaking the balance.
- **No more double payments for bank users.** If a card's bill is paid from a bank-synced account, the real payment already arrives with your bank sync — so "Pay Bill" now tells you to pay in your bank app instead of logging a second payment, and auto-pay switches itself off for that account.
- **Clear heads-ups while adding a transaction:** one if its date is before the account's starting date (so it won't move the balance), and one if you're typing a transaction by hand into an account that already syncs from your bank (it would be counted twice).
- The Home balance chart now follows exactly the same rules as your balances, so its last point always equals your total.
